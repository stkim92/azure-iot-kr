...
이 문서는 [Keil Compiler](https://www.keil.com/)를 기반으로 WIZnet IoT Shield(앰투앰넷 Cat.M1 모듈)와 STM32L496 Nucleo-144를 이용하여 X-CUBE-AZURE SDK 개발 환경을 구축하는 방법에 대하여 설명합니다.
...
- **Software**
  ...
  - [X-CUBE-AZURE SDK](https://github.com/Wiznet/azure-iot-kr/tree/master/samples/LTE/Cat.M1/en.x-cube-azure_v2)
...
{
  /* 중략 */
  // tom 19.11.21 Add uartHandler(LPUART1) to use Virtual Port
...
{
  /* 중략 */
  else if(uartHandle->Instance==USART3) //tom 19.12.02
  {
    /* Peripheral clock disable */
  ...
  /* USER CODE BEGIN USART1_MspDeInit 1 */
	HAL_NVIC_DisableIRQ(USART3_IRQn); //tom 19.12.02
...
```cpp
 ...
 memcpy(cellular_params.sim_slot[1].apn, "", (size_t)sizeof("")); //tom 19.11.19 for SK telecom
 
 /* Initialize all other fields */
 //cellular_params.set_pdn_mode = 1U;
 cellular_params.set_pdn_mode = 3U; //edited by tom 19.11.14 pdn ipv4v6
...
int tcpsocketconnection_connect(TCPSOCKETCONNECTION_HANDLE tcpSocketConnectionHandle, const char* host, const int port)
{
  ...
  /* 중략 */
  // tom 19.11.28 기존 SDK의 경우 IPv4로 처리하지만 WIoT-QC01는 DNS Query를 수행하면 IPv6를 return 때문에,
  // 도메인 주소에 맞는 IPv4 IP주소를 static하게 처리하였음
...
ADC_HandleTypeDef hadc1; //tom 19.11.29
uint16_t value;
uint16_t vout;
float temp1;
...
/* 중략 */
          ...
          if (SERIALIZE(&destination, &destinationSize,
                mdl->mac, mdl->TEMPERATURE, //tom 19.11.28 Add sensing data
          ...
          else
          {
			...
			mdl->TEMPERATURE=temp1; //tom 19.11.27
...
Step 1. 필수 구성 요소의 Software란의 [X-CUBE-AZURE SDK](https://github.com/Wiznet/azure-iot-kr/tree/master/samples/LTE/Cat.M1/en.x-cube-azure_v2)를 Local 저장소에 저장하고, 다음 경로 `azure-iot-kr-master\samples\LTE\Cat.M1\en.x-cube-azure_v2\AZURE_V1.2.0\Projects\STM32L496ZG-Nucleo\Applications\Cloud\Azure\MDK-ARM`에 위치한 Keil 프로젝트를 엽니다.
...
빌드가 성공적으로 완료되면 `project.boardname.bin` 파일이 생성되어 download 됩니다.
